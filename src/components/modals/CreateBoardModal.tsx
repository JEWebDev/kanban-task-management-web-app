...
function AddNewBoard() {
  ...
  const isCreateMode = modal === "create-board";
  const columnNames =
  ...
  const mode = isCreateMode ? "create-board" : "edit-board";
  ...
  return (
    ...
    >
      <h2 className="mb-6 heading-l">
        {isCreateMode ? "Add New Board" : "Edit Board"}
      </h2>
      <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-2">
          <TextInput
            ...
            defaultValue={isCreateMode ? "" : board?.name}
          />
        </div>

        <SubtasksForm
          label={isCreateMode ? "Columns" : "Board Columns"}
          name="columnNames"
          placeholder="e.g. Todo"
          defaultValues={isCreateMode ? [] : columnNames}
        />
